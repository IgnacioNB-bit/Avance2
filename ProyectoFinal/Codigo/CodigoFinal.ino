...
const int stepsFor90Degrees = stepsPerRevolution / 14; // Pasos para 90 grados
...
void loop() {
  if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
    ...
    // Alternar el sentido de giro (invertido)
    if (readCount % 2 == 0) {
      ...
      moveMotor(stepsFor90Degrees + 10, false); // Gira 90 grados en sentido antihorario
...
void stepMotor(bool clockwise) {
  if (!clockwise) { // Sentido horario (invertido)
    ...
    delay(5);
    digitalWrite(in4, LOW);
  } else { // Sentido antihorario (invertido)
    digitalWrite(in4, HIGH);
    delay(5); // Aumentar el retardo
    digitalWrite(in4, LOW);
    digitalWrite(in3, HIGH);
